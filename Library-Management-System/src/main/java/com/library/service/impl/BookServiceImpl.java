...
public class BookServiceImpl implements BookService{
    ...
    public BookDto borrowedBook(Long bookId, Long borrowerId) {
        Book book = bookRepository.findById(bookId).orElseThrow(()->new NotFoundException("Book doesn't exist"));
        Borrower borrower = borrowerRepository.findById(borrowerId).orElseThrow(()->new NotFoundException("Borrower doesn't exist"));
    ...
    public BookDto returnBook(Long bookId, Long borrowerId) {
        Book book = bookRepository.findById(bookId).orElseThrow(()->new NotFoundException("Book doesn't exist"));
        Borrower borrower = borrowerRepository.findById(borrowerId).orElseThrow(()->new NotFoundException("Borrower doesn't borrow this book"));
        if(!borrower.getBooks().contains(book)){
